...
#    * configure driver = { type: 'chrome', addOptions: ['--incognito', '--disable-geolocation'] }
  ...
  @mobile_bus_listing
  Scenario: Bus Listing without login NueGo Web test
    # Background URL
...
#    * maximize()
    ...
    * delay(2000)
    And input('#search-source-mobile', 'New Delhi',50)
    And input('#search-destination-mobile', 'Amritsar',50)
    And click("//div[@class='Tomorrow']")
    * delay(1000)
    And click("//img[@class='plus']")
    And click("//div[@class='searct-btn']")
